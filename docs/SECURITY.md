...
| 13 | **Invitation content injection (XSS via teacher-supplied values)** | A caller-supplied value (teacher name, temporary password) lands in the rendered invitation content. Without escaping, a crafted value could inject markup into the copyable invitation message. | The invitation content is rendered server-side (`lib/email/templates/*`); every dynamic value (teacher name, temporary password) is HTML-escaped via `escapeHtml` (`lib/email/sanitize.ts`) before it enters the rendered output. There is no outbound email transport — the rendered invitation is shown in the admin UI for the admin to copy and deliver out-of-band. Tested by `tests/unit/teacher-invite-email.test.ts`. |
