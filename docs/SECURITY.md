...
| 13 | **Email header injection (CWE-93, Phase 6)** | A caller-supplied value (teacher name, doc-type name) lands in the `To`, `Subject`, or `From` header. A CR/LF in that value could smuggle an extra `Bcc:` header on a vulnerable transport, or break out of the MIME envelope. Multi-recipient sends would be possible. | `lib/email/sanitize.ts` validates every field that enters the outbound payload: rejects `\r`/`\n`/other control bytes, caps lengths, requires a single plain RFC-5321-shaped address for `to` (no commas, no angle-brackets, no display names). All sends route through the single `sendEmail` dispatcher (`lib/email/send.ts`); the `EmailMessage` type forbids `cc`/`bcc`/`replyTo`/`attachments` at the type level. Tested by `tests/unit/reminders/header-injection.test.ts` (27 cases) + `tests/unit/reminders/send.test.ts` (sanitisation runs BEFORE any provider call). |
| 14 | **Email provider key leakage (Phase 6)** | `RESEND_API_KEY` accidentally referenced from a client component (e.g. via an ill-prefixed `NEXT_PUBLIC_` variant) or interpolated into a string that ends up in `.next/static/**`. | `scripts/leakage-grep.mjs` extended in Phase 6: sentinel-string scan over `.next/static/**` (`RESEND_API_KEY`) + literal-value sweep when the key is set in the environment. Adapter throws on construction if `EMAIL_PROVIDER=resend` and `RESEND_API_KEY` is missing — fail-loud rather than silent no-op. |
| 15 | **Reminder email leaking cross-teacher data (Phase 6)** | A template that takes a richer context could accidentally include another teacher's name, the storage key of the rejected doc, the rejection reason verbatim, or an embedded auth bypass token. | §11.3 enforced structurally: template context types in `lib/email/templates/base.ts` admit ONLY first name + doc-type name + portal URL. Storage keys, file IDs, and rejection reasons are not reachable through the type system. The dispatcher's privacy-payload tests assert on the mock-captured `EmailMessage` (no S3/R2/Supabase URLs, no `teachers/<uuid>/` paths, no `jwt=`/`session_token=`, no `<img>` tags). Manual-send route never accepts an email from the body — recipient is looked up from `users.email` of the supplied `teacherId`. |
